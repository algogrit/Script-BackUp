...
- This `README.md`.
- `update_scripts.sh` - Use to backup the sytem to this folder. Also snapshots iTerm2 prefs into `iTerm2/`, Claude Code settings into `Claude/` (`settings.json`, `keybindings.json`, `CLAUDE.md`, `commands/`, `agents/`, and `plugins/known_marketplaces.json`), and Codex settings into `Codex/` (`config.toml`, `AGENTS.md`, and user skills). `~/.claude.json` and Claude session/project state are intentionally excluded (they hold oauth/identity and machine-local history). Codex `auth.json`, `installation_id`, `history.jsonl`, `sessions/`, `cache/`, `.tmp/`, `tmp/`, `shell_snapshots/`, SQLite state, `models_cache.json`, `version.json`, `plugins/`, and `skills/.system` are also excluded.
