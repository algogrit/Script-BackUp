...
- This `README.md`.
- `update_scripts.sh` - Use to backup the sytem to this folder. Also snapshots iTerm2 prefs into `iTerm2/` and Claude Code settings into `Claude/` (`settings.json`, `keybindings.json`, `CLAUDE.md`, `commands/`, `agents/`, and `plugins/known_marketplaces.json`). `~/.claude.json` and session/project state are intentionally excluded (they hold oauth/identity and machine-local history).
- `restore_scripts.sh` - Copies scripts from this folder to the original, filesystem locations. Applies the System Settings tweaks (via `apply_system_settings.sh`), restores iTerm2 prefs + enables iTerm2's native custom-folder sync, and restores Claude Code settings from `Claude/` into `~/.claude/`. Quit iTerm2 before running.
