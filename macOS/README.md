...
- This `README.md`.
- `update_scripts.sh` - Use to backup the sytem to this folder. Also snapshots iTerm2 prefs into `iTerm2/`, Claude Code settings into `Claude/` (`settings.json`, `keybindings.json`, `CLAUDE.md`, `commands/`, `agents/`, `skills/`, `hooks/`, and `plugins/{known_marketplaces,installed_plugins}.json`), and Codex settings into `Codex/` (`config.toml`, `AGENTS.md`, `rules/`, `prompts/`, and user skills). `~/.claude.json` and Claude session/project state are intentionally excluded (they hold oauth/identity and machine-local history). Codex `auth.json`, `installation_id`, `history.jsonl`, `sessions/`, `cache/`, `.tmp/`, `tmp/`, `shell_snapshots/`, SQLite state, `models_cache.json`, `version.json`, `plugins/`, and `skills/.system` are also excluded.
- `restore_scripts.sh` - Copies scripts from this folder to the original, filesystem locations. Applies the System Settings tweaks (via `apply_system_settings.sh`), restores iTerm2 prefs + enables iTerm2's native custom-folder sync, restores Claude Code settings from `Claude/` into `~/.claude/`, and restores Codex settings from `Codex/` into `~/.codex/`. Quit iTerm2 before running.
